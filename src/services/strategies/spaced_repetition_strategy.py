...
class SpacedRepetitionStrategy(RecommendationStrategy):
    def recommend(self, user_id, max_exercises):
        ...
        self.logs_df = self.logs_df.merge(self.feature_vectors.metadata[['question_id', 'item_id']], left_on='question_id',
        ...
        for item, group_df in self.logs_df.groupby('item_id'):
            ...
            if len(m_values) == 0:
                ...
                F_missed = sum([4 * m_values[-1], 2 * m_values[-2] if len(m_values) > 1 else 0,
                                m_values[-3] if len(m_values) > 2 else 0, m_before])
        ...
        if recommendations["exercise_ids"]:
            ...
            if max_f_missed == -1:
                recommendations[
                    "message"] = f"{hi_message} Hãy thử sức với những câu hỏi thuộc các chủ đề: {concepts_message} mà cậu chưa từng gặp trên ScoreUp nhé"
            elif max_f_missed < 4:
                recommendations[
                    "message"] = f"{hi_message} Đây là các chủ đề cậu thường xuyên trả lời sai: {concepts_message}. Hãy luyện tập lại nhé!"
            elif max_f_missed < 8:
                recommendations[
                    "message"] = f"{hi_message} Cậu đôi lúc gặp chút khó khăn khi trả lời câu hỏi thuộc chủ đề: {concepts_message}. Cùng ScoreUp ôn tập lại nhé!"
            elif max_f_missed == 8:
                recommendations[
                    "message"] = f"{hi_message} Đây là chủ đề cậu gặp nhiều khó khăn và trả lời sai nhiều nhất: {concepts_message}. Hãy cố gắng hơn nữa nhé!"
        else:
